...
- (void)refreshVisibleCellAppearance {
    ...
    if ((_minimumPageAlpha == 1.0 && _minimumPageScale == 1.0) || _pageSize.width <= 0 || _pageSize.height <= 0) {
        return;//无需更新
    }
    switch (self.orientation) {
        case NewPagedFlowViewOrientationHorizontal:{
...
- (void)singleCellTapAction:(UIGestureRecognizer *)gesture {
    ...
    if ([self.delegate respondsToSelector:@selector(didSelectCell:withSubViewIndex:inFlowView:)]) {
        
        [self.delegate didSelectCell:gesture.view withSubViewIndex:gesture.view.tag inFlowView:self];
